...
image_dir = "/media/yuganlab/blackstone/HZL/Retinal_seg/Retina-Blood-Vessel-Segmentation-in-PyTorch/data/test/image"
mask_dir  = "/media/yuganlab/blackstone/HZL/Retinal_seg/Retina-Blood-Vessel-Segmentation-in-PyTorch/data/test/mask"
...
generator = torch.Generator(device="cuda:1").manual_seed(24)
tokenizer = T5Tokenizer.from_pretrained("t5-base")
text_encoder = T5EncoderModel.from_pretrained("t5-base").to("cuda:1")
...
def evaluate(config, epoch, pipe):
    ...
    images = pipe(prompt="malignant tumor", image=test_mask_tensor.to("cuda:1")).images
...
def train_loop(config, controlnet, noise_scheduler, optimizer, train_dataloader, lr_scheduler):
    ...
    for epoch in range(config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            timesteps = torch.randint(0, scheduler.num_train_timesteps, (bs,), device=clean_images.device).long()
            ...
            with accelerator.accumulate(controlnet):
                ...
                print("torch shape: ", seg_masks.shape)
                noise_pred = controlnet(
                    hidden_states=seg_masks,
                    controlnet_cond=timesteps, 
                    conditioning_scale=0.5,
                    encoder_hidden_states=seg_masks
                ).images[0]
